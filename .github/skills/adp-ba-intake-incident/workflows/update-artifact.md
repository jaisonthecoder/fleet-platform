...
- Confirm the existing artifact plus the changed incident, impact, root cause, corrective action, validation, or owner evidence.
...
2. Update only the affected incident source, impact, root cause, corrective action, validation, risk, or owner sections.
3. Preserve prior decision traceability and record what changed.
4. Tie every material update to source evidence such as an incident update, postmortem revision, ticket, telemetry, or owner confirmation.
...
## Anti-patterns
- Rewriting the whole artifact for style when only root cause status changed.
- Leaving stale validation expectations after corrective action changes.
- Updating scope without routing SRE, support, quality, or security uncertainty.
