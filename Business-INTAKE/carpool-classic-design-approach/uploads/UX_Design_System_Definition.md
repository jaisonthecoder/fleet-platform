...
The visual language borrows from the working port, not from generic SaaS:
...
- **Re-theming = changing Layer 1.** Tints, hovers, focus rings and soft backgrounds are computed with `color-mix(in oklab, …)`, so a tenant brand (AD Ports teal, or any client color) propagates through every component automatically. No component CSS is ever edited for a re-brand.
