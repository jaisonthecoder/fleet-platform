---
name: adp-arch-domain
description: "Ubiquitous language, glossary, bounded contexts/modules, ownership, entities, relationships, invariants, lifecycle states, solution boundaries. Use when producing or updating \\\"docs/03-architecture/DOMAIN/domain-model.md, docs/03-architecture/DOMAIN/domain-*.drawio\\\". Owned by AI Solution Architect."
---
...
- **why_critical:** Prevents invented business objects, unclear ownership, ambiguous terminology, and wrong service/module boundaries.
- **default_prompt:** Use the adp-arch-domain skill. Open SKILL.md, choose the matching workflow, and complete the request with evidence.
- **short_description:** Ubiquitous language, glossary, bounded contexts/modules
...
## Why critical
Prevents invented business objects, unclear ownership, ambiguous terminology, and wrong service/module boundaries.

## Purpose
Ubiquitous language, glossary, bounded contexts/modules, ownership, entities, relationships, invariants, lifecycle states, solution boundaries.
...
- **Primary artifact focus:** docs/03-architecture/DOMAIN/domain-model.md, docs/03-architecture/DOMAIN/domain-*.drawio.
...
## Domain Model Diagram Contract

When creating or updating `docs/03-architecture/DOMAIN/domain-model.md`, model the domain as a UML/class-diagram style artifact. The model must identify domain classes/entities, key attributes, aggregate roots where relevant, relationships, cardinality, ownership boundaries, lifecycle states, and important invariants. Prefer a Mermaid `classDiagram` in Markdown for text-native output, and use `docs/03-architecture/DOMAIN/domain-*.drawio` when a visual diagram file is required. Do not substitute a process flow, component diagram, ERD-only view, or generic glossary for the domain model unless the user explicitly asks for that separate artifact.

For Draw.io domain diagrams, use class-diagram notation rather than generic boxes. Each domain concept must be rendered as a UML-style class shape with compartments:

- **Header compartment:** class/entity name plus stereotype such as `<<AggregateRoot>>`, `<<Entity>>`, `<<ValueObject>>`, `<<DomainService>>`, or `<<ExternalSystem>>`.
- **Attribute compartment:** key domain attributes with types when known, identity fields, required/optional markers where useful, and business-significant states.
- **Operation/invariant compartment:** only include domain behaviors, commands, lifecycle transitions, or invariants that affect ownership or implementation.
- **Relationships:** use association, aggregation, composition, dependency, or inheritance connectors with labels and multiplicities such as `1`, `0..1`, `0..*`, `1..*`; show direction and ownership where meaningful.
- **Boundaries:** use containers only for bounded contexts/modules; do not represent entities as plain rectangles.

When hand-authoring `.drawio`, prefer diagrams.net-compatible UML class construction: swimlane/class-style cells with stacked child rows for attributes and behaviors, plus labeled edges for multiplicity and relationship type. If a class has no confirmed attributes yet, include an explicit placeholder row such as `attributes: TBD from source evidence` rather than collapsing it into a plain box.
...
- **When using adp-arch-domain:** finish with the files changed and the evidence a reviewer should inspect first.

## Decision checkpoints

- **Before producing docs/03-architecture/DOMAIN/domain-model.md, docs/03-architecture/DOMAIN/domain-*.drawio:** confirm the artifact is owned by this skill and not a neighboring role.
...
- A new instance of `docs/03-architecture/DOMAIN/domain-model.md, docs/03-architecture/DOMAIN/domain-*.drawio` is required.
...
## Outputs
- docs/03-architecture/DOMAIN/domain-model.md, docs/03-architecture/DOMAIN/domain-*.drawio
...
- **Generated artifact:** `docs/03-architecture/DOMAIN/domain-model.md`, `docs/03-architecture/DOMAIN/domain-*.drawio`
...
- **Downstream:** `ai-backend-dotnet`, `ai-frontend-react`, `ai-data-engineer`, `ai-platform-engineer`, `ai-security-engineer`.
