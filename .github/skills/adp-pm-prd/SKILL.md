---
name: adp-pm-prd
description: "Product Requirements Document aligned to ISO/IEC/IEEE 29148 product, system, and software requirements concepts. Use when producing or updating docs/02-product/PRD/prd.md, deriving product requirements from BRD, defining behavior, rules, flows, acceptance criteria, NFRs, or dedicated user story files."
---
...
- **why_critical:** The product contract every downstream role reads; AI uses it as the most-loaded PM artifact for design, build, test, review, and release context.
- **default_prompt:** Use the adp-pm-prd skill. Open SKILL.md, choose the matching workflow, and complete the request with evidence.
- **short_description:** ISO-aligned product requirements and acceptance criteria

**Owner role:** AI Product Manager (`ai-product-manager`)
**Primary artifact:** docs/02-product/PRD/prd.md

## Why critical
The product contract every downstream role reads; AI uses it as the most-loaded PM artifact for design, build, test, review, and release context.

## Purpose
Product Requirements Document aligned to ISO/IEC/IEEE 29148 product, system, and software requirements concepts: product scope, behavior, rules, flows, derived requirements, acceptance criteria, measurable NFRs, and user-story validation evidence.
...
You own the PRD and the backlog. Translate business requirements into buildable, testable product requirements with ruthless prioritization.
...
- **Primary artifact focus:** docs/02-product/PRD/prd.md.
...
6. **Start from the bundled template.** Use `templates/prd-template.md.tmpl` for the `docs/02-product/PRD/prd.md` index and `templates/feature-template.md.tmpl` for every dedicated user story file under `docs/02-product/PRD/user-stories/<US-ID>-<slug>.md`. If the template does not fit, state why in the deviation log before deviating.
...
8. **Every story gets a dedicated file during PRD writing.** User stories must not live only as a large embedded section in `docs/02-product/PRD/prd.md`. `docs/02-product/PRD/prd.md` summarizes or indexes stories and links to one file per story under `docs/02-product/PRD/user-stories/<US-ID>-<slug>.md`.
9. **Every story file carries validation instructions and evidence placeholders.** Each story file must include story ID, title, owner, priority, BR mapping, "As a / I want / So that", acceptance criteria, API validation examples or a not-applicable rationale, UI validation examples or a not-applicable rationale, implementation validation checklist, and a test evidence table.
10. **User story scope verification is mandatory and scope-dependent.** Classify each story as backend/API, UI, full-stack, or non-API/non-UI. Backend/API stories require runnable `curl` commands and expected status/response fields. UI stories require a Playwright test outline for the primary journey. Full-stack stories require both. Non-API/non-UI stories require a rationale and an alternate evidence path.
...
- **When using adp-pm-prd:** finish with the files changed and the evidence a reviewer should inspect first.

## Decision checkpoints

- **Before producing `docs/02-product/PRD/prd.md`:** confirm the artifact is owned by this skill and not a neighboring role.
...
- A new instance of `docs/02-product/PRD/prd.md` is required.
...
## Outputs
- `docs/02-product/PRD/prd.md` — the PRD; includes problem, goals and non-goals, target users, user story index, dependencies, assumptions, and links to dedicated user story files.
- `docs/02-product/PRD/user-stories/<US-ID>-<slug>.md` — one file per user story. Carries BR mapping, acceptance criteria, user story scope verification, backend/API `curl` checks where applicable, UI Playwright checks where applicable, implementation validation checklist, and evidence placeholders.
- Acceptance criteria — authored in each dedicated user story file.
- Non-functional requirements — cross-cutting NFRs in `docs/02-product/PRD/prd.md`; story-specific NFRs in the relevant user story file.
- Scoped/clarified ticket — output of `scope-ambiguous-ticket.md` (a story added to `docs/02-product/PRD/prd.md` §6 release scope, or an explicit rejection).
...
- `workflows/write-prd.md` — produce `docs/02-product/PRD/prd.md` plus one dedicated user story file per story.
- `workflows/define-acceptance.md` — author or refine acceptance criteria in each story's dedicated file.
- `workflows/define-nfrs.md` — author or refine NFRs (cross-cutting in `docs/02-product/PRD/prd.md`, story-specific in the user story file).
- `workflows/scope-ambiguous-ticket.md` — turn a vague request into a story added to `docs/02-product/PRD/prd.md` §6 release scope, or an explicit rejection.

## References

Load only the references needed for the current request:

- `references/guidance.md` — PRD-specific decisions (granularity, traceability, status transitions, AC quality, when to route out of the PRD).

## Templates

Use these bundled templates when the request produces or updates the primary artifact:

- `templates/prd-template.md.tmpl` — `docs/02-product/PRD/prd.md` index and product contract.
- `templates/feature-template.md.tmpl` — dedicated user story file under `docs/02-product/PRD/user-stories/<US-ID>-<slug>.md`.
...
- **Required inputs:** `docs/01-discovery/brd.md`, `business outcome`, `constraints`, `success metric`
- **Generated artifacts:** `docs/02-product/PRD/prd.md` plus one dedicated user story file per story under `docs/02-product/PRD/user-stories/`.
