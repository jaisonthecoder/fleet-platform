...
6. Keep assumptions and open questions separated when updating the artifact:
   - Assumptions belong in `## 7. Assumptions` with validation needed and impact if wrong.
   - Open questions belong in `## 8. Open questions` and must remain highlighted with owner, needed-by date, blocker status, and latest-answer status.
   - Use a short chat conversation with the user to close open questions during the update whenever the missing answer can be obtained immediately.
