---
name: adp-bknd-nest-worker
description: "Proactive slice: scheduled job (@nestjs/schedule, BullMQ repeatable), batch process, polling, publishers, lock/concurrency, tests. Use when producing or updating \\\"backend Nest source, backend tests, runbook updates\\\". Owned by AI Backend Engineer (NestJS)."
---
...
- **why_critical:** Makes background behavior explicit, observable, and recoverable instead of buried in ad hoc jobs.
- **default_prompt:** Use the adp-bknd-nest-worker skill. Open SKILL.md, choose the matching workflow, and complete the request with evidence.
- **short_description:** Proactive slice: scheduled job , batch process, polling

**Owner role:** AI Backend Engineer (NestJS) (`ai-backend-nestjs`)
**Primary artifact:** backend Nest source, backend tests, runbook updates

## Why critical
Makes background behavior explicit, observable, and recoverable instead of buried in ad hoc jobs.

## Purpose
Proactive slice: scheduled job (@nestjs/schedule, BullMQ repeatable), batch process, polling, publishers, lock/concurrency, tests.
...
- **Primary artifact focus:** backend Nest source, backend tests, runbook updates.
...
- **When using adp-bknd-nest-worker:** finish with the files changed and the evidence a reviewer should inspect first.

## Decision checkpoints

- **Before producing backend Nest source, backend tests, runbook updates:** confirm the artifact is owned by this skill and not a neighboring role.
...
- A new instance of `backend Nest source, backend tests, runbook updates` is required.
...
## Outputs
- backend Nest source, backend tests, runbook updates
...
- **Generated artifact:** `backend Nest source`, `backend tests`, `runbook updates`
