---
name: adp-bknd-nest-crosscut
description: "Auth (Passport/JWT, RBAC/ABAC guards), interceptors, pipes, filters, distributed tracing, structured logging, correlation IDs, error handling, health checks, config, secrets, feature flags, rate limiting, OpenAPI/Swagger. Use when producing or updating \\\"backend Nest source code\\\". Owned by AI Backend Engineer (NestJS)."
---
...
- **why_critical:** Keeps enterprise technical behavior consistent instead of repeating it differently in every backend feature slice.
- **default_prompt:** Use the adp-bknd-nest-crosscut skill. Open SKILL.md, choose the matching workflow, and complete the request with evidence.
- **short_description:** Auth , interceptors, pipes, filters, distributed tracing

**Owner role:** AI Backend Engineer (NestJS) (`ai-backend-nestjs`)
**Primary artifact:** backend Nest source code

## Why critical
Keeps enterprise technical behavior consistent instead of repeating it differently in every backend feature slice.

## Purpose
Auth (Passport/JWT, RBAC/ABAC guards), interceptors, pipes, filters, distributed tracing, structured logging, correlation IDs, error handling, health checks, config, secrets, feature flags, rate limiting, OpenAPI/Swagger.
...
- **Primary artifact focus:** backend Nest source code.
...
- **When using adp-bknd-nest-crosscut:** finish with the files changed and the evidence a reviewer should inspect first.

## Decision checkpoints

- **Before producing backend Nest source code:** confirm the artifact is owned by this skill and not a neighboring role.
...
- A new instance of `backend Nest source code` is required.
...
- **Generated artifact:** `backend Nest source code`
