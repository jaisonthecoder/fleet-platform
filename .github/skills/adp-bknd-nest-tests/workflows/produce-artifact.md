...
## Before you start
- Confirm the request is for `backend Nest test files (*.spec.ts, *.e2e-spec.ts), integration/contract/E2E test suites, coverage/mutation evidence`.
- Confirm the work is owned by AI Backend Engineer (NestJS) (`ai-backend-nestjs`).
